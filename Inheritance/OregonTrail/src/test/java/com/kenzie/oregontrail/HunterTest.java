...
public class HunterTest {
    ...
    void canUpdateHealth() {
        try {
            ...
            eat.invoke(hunter);
            assertEquals(TEST_NO_FOOD, (int)getFood.invoke(hunter), "Hunter.food after eating is 0" );
            assertEquals(TEST_ISHEALTHY, (boolean)getIsHealthy.invoke(hunter), "Hunter.isHealthy after eating once should be true" );

            eat.invoke(hunter);
            assertEquals(TEST_NO_FOOD, (int)getFood.invoke(hunter), "Hunter.food after overeating is still 0" );
            assertEquals(TEST_ISNOTHEALTHY, (boolean)getIsHealthy.invoke(hunter), "Hunter.isHealthy after eating twice should be false" );
        }
        catch(Exception e){
            System.out.println(e.getMessage());
